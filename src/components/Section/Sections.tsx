  ...
  return (
    <section
      className='flex flex-col w-full items-center justify-center pt-6 
        bg-white lg:w-[600px] lg:mt-8 lg:items-start lg:mb-[35px] lg:drop-shadow-lg'
    >
        ...
        >
          {
            query.data?.sections.map(({ id, name, items }) => (
              <AccordionItem key={id} value={`${id}`} className="font-medium text-2xl text-color-dark">
                <AccordionTrigger onClick={() => handleSectionSelect(id)}>{name}</AccordionTrigger>
                {
                  items.map(({ id, name, images, description, price }) => (
                    <AccordionContent key={id} >
                      <ListItem name={name} images={images} description={description} price={price} />
                    </AccordionContent>
                  ))
                }
              </AccordionItem>
