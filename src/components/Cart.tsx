          ...
          <Dialog>
            ...
            <DialogContent className="p-0 h-full">
              ...
              <div className="flex flex-col w-full h-full bg-white">
                <ScrollArea className={`${cart.items.length < 2 ? 'h-[267px]' : 'h-[80%]'}`}>
                  {cart.items.map((item) => (
                    <div className={`flex flex-col pt-2 even:border-b border-[#DADADA] ${cart.items.length > 1 ? 'odd:border-b' : ''}`} key={item.id}>
                      <div className="flex flex-col px-4">
                        <div className="flex items-center justify-between mb-1">
                          <p className="text-base text-color-dark">{item.name}</p>
                          <p className="text-base font-medium">R${item.price.toFixed(2)}</p>
                ...
                <div className="flex h-full p-4 pb-32 w-full items-start justify-between bg-[#F8F9FA] border-t border-t-[#DADADA]">
                  <p className="text-2xl font-light text-color-dark">Total:</p>
                  <p className="text-2xl font-bold text-color-dark">R${cart.totalPrice.toFixed(2)}</p>
                </div>
              </div>
